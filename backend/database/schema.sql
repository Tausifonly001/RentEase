...
create policy deliveries_insert_own on public.deliveries for insert to authenticated with check (auth.uid() = user_id);

drop policy if exists deliveries_update_own on public.deliveries;
create policy deliveries_update_own on public.deliveries for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);
