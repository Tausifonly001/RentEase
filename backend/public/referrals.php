...
<main class="pt-20">
        ...
        <div class="max-w-7xl mx-auto px-8 grid grid-cols-1 md:grid-cols-2 gap-12 items-center relative z-10">
            <div class="text-left text-white">
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    <button onclick="document.getElementById('referral-link-section').scrollIntoView({behavior: 'smooth'})" class="bg-secondary text-white px-8 py-4 rounded-lg font-normal hover:opacity-90 transition-all shadow-lg active:scale-95 text-center">Invite Friends Now</button>
            ...
            <div class="relative hidden md:block">
                ...
                <div class="absolute -bottom-6 -left-6 bg-secondary p-6 rounded-lg shadow-xl">
                    ...
                    <p class="text-teal-100 text-[10px] font-normal tracking-widest uppercase">AVERAGE SAVINGS ANNUALLY</p>
                </div>
            </div>
        </div>
    ...
    <section id="referral-link-section" class="py-16 bg-white">
        <div class="max-w-3xl mx-auto px-8">
            <div class="bg-slate-50 border border-slate-200 p-8 md:p-12 rounded-xl shadow-sm">
                <h2 class="text-2xl font-normal text-slate-900 mb-4 text-center">Share Your Unique Link</h2>
                ...
                <div class="flex flex-col md:flex-row gap-4 mb-8">
                    <div class="flex-grow flex items-center bg-white border border-slate-200 rounded-lg px-4 py-3 focus-within:ring-2 focus-within:ring-secondary/20 focus-within:border-secondary transition-all">
                        <span class="text-slate-600 text-sm font-light select-all">rentease.com/signup?ref=<?= htmlspecialchars((string)($stats['referral_code'] ?? '')) ?></span>
                    </div>
                    <button onclick="navigator.clipboard.writeText('rentease.com/signup?ref=<?= htmlspecialchars((string)($stats['referral_code'] ?? '')) ?>')" class="bg-primary text-white px-8 py-3 rounded-lg font-normal hover:bg-slate-800 transition-all flex items-center justify-center gap-2 whitespace-nowrap">
    ...
    <section class="py-16 bg-slate-50">
        <div class="max-w-7xl mx-auto px-8">
            <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    <h2 class="text-3xl font-normal text-primary mb-2">Your Referral Dashboard</h2>
                    <p class="text-slate-500 text-lg">Track your rewards and friend status in real-time.</p>
                </div>
                <button class="text-secondary font-normal flex items-center gap-1 hover:underline">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="bg-white p-8 rounded-xl shadow-sm border border-slate-100 flex flex-col items-center text-center">
                    ...
                    <p class="text-[10px] font-normal tracking-widest text-slate-400 mb-1 uppercase">TOTAL REWARDS EARNED</p>
                    <h3 class="text-4xl font-normal text-primary">$<?= number_format((float)($stats['total_earned'] ?? 0), 2) ?></h3>
                    <p class="text-sm text-secondary font-light mt-2"><?= $stats['successful_referrals'] ?? 0 ?> Rewards Applied</p>
                </div>
                <div class="bg-white p-8 rounded-xl shadow-sm border border-slate-100 flex flex-col items-center text-center">
                    ...
                    <p class="text-[10px] font-normal tracking-widest text-slate-400 mb-1 uppercase">FRIENDS INVITED</p>
                    <h3 class="text-4xl font-normal text-primary"><?= $stats['friends_invited'] ?? 0 ?></h3>
                    <p class="text-sm text-slate-500 mt-2 font-light"><?= $stats['pending_invitations'] ?? 0 ?> Invitations Pending</p>
                </div>
                <div class="bg-white p-8 rounded-xl shadow-sm border border-slate-100 flex flex-col items-center text-center">
                    <div class="w-12 h-12 bg-teal-50 rounded-full flex items-center justify-center mb-6">
                        <span class="material-symbols-outlined text-secondary">check_circle</span>
                    </div>
                    <p class="text-[10px] font-normal tracking-widest text-slate-400 mb-1 uppercase">SUCCESSFUL REFERRALS</p>
                    <h3 class="text-4xl font-normal text-primary"><?= $stats['successful_referrals'] ?? 0 ?></h3>
                    <p class="text-sm text-secondary font-light mt-2">+<?= $stats['recent_referrals'] ?? 0 ?> in the last 30 days</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-24 bg-white">
        <div class="max-w-7xl mx-auto px-8">
            <h2 class="text-3xl font-normal text-primary text-center mb-16">How it Works</h2>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-12">
                <div class="text-center relative">
                    <div class="w-16 h-16 bg-primary text-white flex items-center justify-center rounded-full mx-auto mb-6 font-normal text-xl relative z-10">1</div>
                ...
                <div class="text-center relative">
                    <div class="w-16 h-16 bg-primary text-white flex items-center justify-center rounded-full mx-auto mb-6 font-normal text-xl relative z-10">2</div>
                    <div class="hidden md:block absolute top-8 left-[60%] w-full h-[2px] bg-slate-100 -z-0"></div>
                ...
                <div class="text-center">
                    <div class="w-16 h-16 bg-secondary text-white flex items-center justify-center rounded-full mx-auto mb-6 font-normal text-xl">3</div>
    ...
    <section class="py-24 bg-slate-50">
        <div class="max-w-4xl mx-auto px-8">
            <h2 class="text-3xl font-normal text-primary text-center mb-16">Common Questions</h2>
            <div class="space-y-4">
                <details class="bg-white p-6 rounded-lg shadow-sm group">
                    <summary class="flex justify-between items-center w-full text-left font-normal text-lg text-primary cursor-pointer list-none">
