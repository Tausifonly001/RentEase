...
<main class="flex-grow w-full max-w-7xl mx-auto px-4 md:px-8 py-10">
    <div class="mb-10 reveal-element">
        <h1 class="text-4xl font-bold text-primary tracking-tight">Welcome Back, <?= htmlspecialchars((string)$currentUser['name']) ?></h1>
                        ...
                        <?php foreach ($rentals as $rental): ?>
                            <div class="group bg-white rounded-2xl border border-slate-100 p-4 shadow-sm hover:shadow-md transition-all">
                                <div class="aspect-[4/3] w-full rounded-xl overflow-hidden bg-slate-100 mb-4 relative">
                                    ...
                                    <div class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm text-primary text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest flex items-center gap-1.5 shadow-sm">
                                ...
                                <h3 class="text-lg font-bold text-slate-900 mb-1"><?= htmlspecialchars($rental['products']['name'] ?? 'Asset #'.$rental['id']) ?></h3>
                                <p class="text-xs text-slate-500 mb-6">Ends: <?= date('M j, Y', strtotime((string)$rental['end_date'])) ?></p>
                                ...
                                <div id="extend-<?= $rental['id'] ?>" class="hidden mt-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                                    <form action="<?= baseUrl('/dashboard') ?>" method="POST" class="flex flex-col gap-3">
                                        ...
                                        <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400">Select Term</p>
                                        <div class="flex gap-2">
                                            <select name="extra_days" class="flex-grow bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-teal-500 outline-none">
                                                <option value="90">3 Months</option>
                                                <option value="180">6 Months</option>
                                                <option value="365">12 Months</option>
                                            </select>
                                            <button type="submit" class="bg-primary text-white px-4 py-2 rounded-lg font-bold text-sm hover:opacity-90">Confirm</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
            ...
            <section class="reveal-element">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <a href="<?= baseUrl('/admin') ?>" class="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col items-center justify-center text-center gap-4 hover:bg-teal-600 transition-all group shadow-lg">
                        ...
                        <span class="text-sm font-bold text-white">Admin Console</span>
                    </a>
                    <?php endif; ?>
                </div>
            </section>
        ...
        <div class="lg:col-span-4 flex flex-col gap-10">
            ...
            <section class="bg-white rounded-2xl border border-slate-100 p-8 shadow-sm reveal-element">
                <div class="flex flex-col items-center text-center gap-4 mb-8">
                    <div class="w-20 h-20 rounded-full bg-slate-100 border-4 border-slate-50 flex items-center justify-center text-2xl font-bold text-primary uppercase">
                        <?= substr((string)$currentUser['name'], 0, 1) ?>
                    </div>
                    <div>
                        <h3 class="text-xl font-bold text-slate-900"><?= htmlspecialchars((string)$currentUser['name']) ?></h3>
                        <p class="text-sm text-slate-500"><?= htmlspecialchars((string)$currentUser['email']) ?></p>
                    </div>
                </div>
                <nav class="space-y-1">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
                    <?php if (empty($maintenanceRequests)): ?>
                        <div class="p-8 text-center">
                            <p class="text-sm text-slate-400 italic">No active service tickets.</p>
                        </div>
                    <?php else: ?>
                            ...
                            <?php foreach (array_slice($maintenanceRequests, 0, 3) as $req): ?>
                                <div class="p-4 hover:bg-slate-50 transition-colors flex items-start gap-4">
                                    <div class="w-8 h-8 rounded-lg <?= $req['status'] === 'RESOLVED' ? 'bg-teal-50 text-teal-600' : 'bg-orange-50 text-orange-600' ?> flex items-center justify-center shrink-0">
                                        <span class="material-symbols-outlined text-sm"><?= $req['status'] === 'RESOLVED' ? 'check_circle' : 'pending' ?></span>
                                    </div>
                                    <div class="flex-grow">
                                        <p class="text-sm font-bold text-slate-900 line-clamp-1"><?= htmlspecialchars($req['issue_description']) ?></p>
                                        <p class="text-[10px] text-slate-400 mt-1 uppercase font-bold tracking-widest"><?= date('M j', strtotime($req['created_at'])) ?> • <?= htmlspecialchars($req['status']) ?></p>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
