...
<main class="max-w-7xl mx-auto px-8 py-16 grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-screen">
    ...
    <div class="lg:col-span-8">
        ...
        <section class="bg-white p-8 rounded-3xl shadow-sm mb-8 border border-slate-100">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div>
                    <span class="text-[10px] font-normal text-secondary uppercase tracking-[0.2em] mb-2 block">Current Schedule</span>
                    <h1 class="text-3xl md:text-4xl font-normal text-primary tracking-tight">Reschedule Delivery</h1>
                </div>
                <div class="flex gap-6 bg-slate-50 p-6 rounded-2xl border border-slate-100">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-[10px] font-normal text-slate-400 uppercase tracking-widest">Date</p>
                            <p class="text-sm font-light text-primary"><?= $delivery ? date('M d, Y', strtotime($delivery['scheduled_date'])) : 'Oct 24, 2024' ?></p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-[10px] font-normal text-slate-400 uppercase tracking-widest">Window</p>
                            <p class="text-sm font-light text-primary"><?= $delivery ? htmlspecialchars((string)($delivery['time_slot'] ?? '09:00 - 12:00')) : '09:00 - 12:00' ?></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <nav class="flex items-center justify-between mb-12 px-8">
            <div class="flex flex-col items-center gap-2 relative step-indicator" data-step="1">
                <div class="step-circle w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center font-normal z-10 shadow-lg shadow-primary/20">1</div>
                <span class="step-text text-[10px] font-normal tracking-widest text-primary uppercase">New Date/Time</span>
            ...
            <div class="flex flex-col items-center gap-2 relative step-indicator" data-step="2">
                <div class="step-circle w-10 h-10 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center font-normal z-10">2</div>
                <span class="step-text text-[10px] font-normal tracking-widest text-slate-400 uppercase">Reason</span>
                <div class="absolute top-5 left-1/2 w-full h-[2px] bg-slate-100 -z-0"></div>
            </div>
            <div class="flex flex-col items-center gap-2 step-indicator" data-step="3">
                <div class="step-circle w-10 h-10 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center font-normal z-10">3</div>
                <span class="step-text text-[10px] font-normal tracking-widest text-slate-400 uppercase">Confirmation</span>
            </div>
        </nav>
        ...
        <div id="step-1-content" class="step-content grid grid-cols-1 md:grid-cols-2 gap-8 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
            ...
            <div>
                <div class="flex items-center justify-between mb-8">
                    <h3 class="text-xl font-normal text-primary tracking-tight"><?= date('F Y') ?></h3>
                ...
                <div class="grid grid-cols-7 text-center gap-y-3 calendar-grid">
                    <?php foreach (['S','M','T','W','T','F','S'] as $day): ?>
                        <span class="text-[10px] font-normal text-slate-400 uppercase tracking-widest py-2"><?= $day ?></span>
                    <?php endforeach; ?>
            ...
            <div class="md:border-l border-slate-100 md:pl-8">
                <h3 class="text-xl font-normal text-primary tracking-tight mb-8">Select Time Window</h3>
                <div class="space-y-6 time-slots">
                    <div>
                        <p class="text-[10px] font-normal text-slate-400 uppercase tracking-widest mb-3">Morning</p>
                        <div class="space-y-3">
                            <button onclick="selectTimeSlot(this)" data-slot="08:00 AM - 11:00 AM" class="time-slot-btn w-full flex items-center justify-between p-4 border border-slate-100 rounded-2xl bg-white hover:border-secondary transition-all group active:scale-95">
                                <span class="text-sm font-light text-primary">08:00 AM - 11:00 AM</span>
                            ...
                            <button onclick="selectTimeSlot(this)" data-slot="09:00 AM - 12:00 PM" class="time-slot-btn w-full flex items-center justify-between p-4 border border-slate-100 rounded-2xl bg-white hover:border-secondary transition-all group active:scale-95">
                                <span class="text-sm font-light text-primary">09:00 AM - 12:00 PM</span>
                                <span class="material-symbols-outlined text-slate-200 group-hover:text-secondary">radio_button_unchecked</span>
                            </button>
                        </div>
                    </div>
                    <div>
                        <p class="text-[10px] font-normal text-slate-400 uppercase tracking-widest mb-3">Afternoon</p>
                        <div class="space-y-3">
                            <button onclick="selectTimeSlot(this)" data-slot="12:00 PM - 03:00 PM" class="time-slot-btn w-full flex items-center justify-between p-4 border border-slate-100 rounded-2xl bg-white hover:border-secondary transition-all group active:scale-95">
                                <span class="text-sm font-light text-primary">12:00 PM - 03:00 PM</span>
                                <span class="material-symbols-outlined text-slate-200 group-hover:text-secondary">radio_button_unchecked</span>
                            </button>
                            <button class="w-full flex items-center justify-between p-4 border border-slate-100 rounded-2xl bg-slate-50 opacity-50 cursor-not-allowed">
                                <span class="text-sm font-light text-slate-400">03:00 PM - 06:00 PM</span>
        ...
        <div id="step-2-content" class="step-content hidden bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
            <h3 class="text-2xl font-normal text-primary mb-8 tracking-tight">Reason for Rescheduling</h3>
            <div class="space-y-6">
                <div>
                    <label class="block text-[10px] font-normal text-slate-400 uppercase tracking-widest mb-3">SELECT REASON</label>
                    <select id="reschedule-reason" class="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-secondary/10 focus:border-secondary outline-none transition-all text-slate-700 font-normal">
                ...
                <div>
                    <label class="block text-[10px] font-normal text-slate-400 uppercase tracking-widest mb-3">ADDITIONAL NOTES (OPTIONAL)</label>
                    <textarea id="additional-notes" rows="4" class="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-secondary/10 focus:border-secondary outline-none transition-all text-slate-700 font-normal" placeholder="Anything else we should know?"></textarea>
                </div>
            </div>
        </div>
        ...
        <div id="step-3-content" class="step-content hidden bg-white p-8 rounded-3xl shadow-sm border border-slate-100 text-center">
            ...
            <h3 class="text-2xl font-normal text-primary mb-4 tracking-tight">Review New Schedule</h3>
            <div class="max-w-md mx-auto bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-8">
                <div class="flex justify-between mb-4">
                    <span class="text-slate-400 font-normal">New Date</span>
                    <span id="summary-date" class="font-normal text-primary">Oct 28, 2024</span>
                </div>
                <div class="flex justify-between mb-4">
                    <span class="text-slate-400 font-normal">Time Window</span>
                    <span id="summary-time" class="font-normal text-primary">09:00 AM - 12:00 PM</span>
                </div>
                <div class="flex justify-between pt-4 border-t border-slate-200">
                    <span class="text-slate-400 font-normal">Service Fee</span>
                    <span class="font-normal text-secondary">$0.00</span>
                </div>
            </div>
            <p class="text-sm text-slate-500 leading-relaxed italic font-light">By confirming, you agree to our rescheduling policy.</p>
        </div>

        <!-- Navigation Buttons -->
        <div class="mt-8 flex justify-between items-center">
            <button onclick="window.location.href='<?= baseUrl('/dashboard') ?>'" class="px-8 py-4 font-normal text-primary bg-white border border-slate-200 rounded-2xl hover:bg-slate-50 transition-all active:scale-95 shadow-sm">
                Cancel
            </button>
            <div class="flex gap-4">
                <button id="prev-btn" onclick="prevStep()" class="px-8 py-4 font-normal text-slate-400 opacity-50 cursor-not-allowed hidden">
                    Previous
                </button>
                <button id="next-btn" onclick="nextStep()" class="px-12 py-4 font-normal bg-primary text-white rounded-2xl shadow-xl shadow-primary/20 hover:opacity-95 transition-all active:scale-95">
                    Next Step
                </button>
            </div>
        </div>
    ...
    <aside class="lg:col-span-4 space-y-8">
        <div class="bg-primary text-white p-8 rounded-3xl shadow-xl relative overflow-hidden group">
            ...
            <ul class="space-y-6 relative z-10">
                <li class="flex items-start gap-4">
                    ...
                    <p class="text-sm text-teal-100/70 leading-relaxed font-light">Cancellations or rescheduling requires at least **24-hour notice**.</p>
                </li>
                <li class="flex items-start gap-4">
                    <span class="material-symbols-outlined text-teal-400 mt-1">payments</span>
                    <p class="text-sm text-teal-100/70 leading-relaxed font-light">One complimentary reschedule is allowed per rental item.</p>
                </li>
                <li class="flex items-start gap-4">
                    <span class="material-symbols-outlined text-teal-400 mt-1">history</span>
                    <p class="text-sm text-teal-100/70 leading-relaxed font-light">Rescheduling within 12 hours may incur a small **$25 service fee**.</p>
                </li>
            </ul>
        </div>
        ...
        <div class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
            <h4 class="text-[10px] font-normal text-slate-400 uppercase tracking-widest mb-6">Item Being Delivered</h4>
            <div class="flex gap-4 items-center">
                ...
                <div>
                    <p class="text-base font-normal text-primary leading-tight"><?= htmlspecialchars((string)($delivery['rentals']['products']['name'] ?? 'Product')) ?></p>
                    <p class="text-xs text-slate-400 font-light mt-1">Order #RE-<?= substr((string)($delivery['order_id'] ?? ''), -5) ?></p>
                </div>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="bg-slate-50 p-8 rounded-3xl border-2 border-dashed border-slate-200 flex flex-col items-center text-center">
            ...
            <p class="text-sm text-slate-500 font-light mb-4">Need help with your delivery?</p>
            <a class="text-secondary font-normal hover:underline" href="<?= baseUrl('/concierge') ?>">Chat with Support</a>
        </div>
    </aside>
</main>
...
function updateStepper() {
    document.querySelectorAll('.step-indicator').forEach(ind => {
        ...
        if (step === currentStep) {
            circle.className = 'step-circle w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center font-normal z-10 shadow-lg shadow-primary/20';
            text.className = 'step-text text-[10px] font-normal tracking-widest text-primary uppercase';
        } else if (step < currentStep) {
            circle.className = 'step-circle w-10 h-10 rounded-full bg-teal-500 text-white flex items-center justify-center font-normal z-10 shadow-lg shadow-teal-500/20';
            text.className = 'step-text text-[10px] font-normal tracking-widest text-teal-600 uppercase';
        } else {
            circle.className = 'step-circle w-10 h-10 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center font-normal z-10';
            text.className = 'step-text text-[10px] font-normal tracking-widest text-slate-400 uppercase';
        }
    });
}
